...
export default function JobsIndex() {
  ...
  return (
    <div className="space-y-4">
      ...
      {jobs.length === 0 ? (
        <div className="rounded-xl border border-dashed p-6 text-center text-sm text-gray-600 dark:text-gray-300 dark:border-gray-700">
          <p>No jobs found.</p>
          <div className="mt-3 flex items-center justify-center gap-2">
            <a href="/profile" className="rounded-md bg-gray-900 px-3 py-1.5 text-white">Upload Base Resume</a>
            <form method="post" action="/api.ingest" className="inline">
              <button className="rounded-md bg-indigo-600 px-3 py-1.5 text-white">Import Jobs</button>
            </form>
          </div>
        </div>
      ) : (
